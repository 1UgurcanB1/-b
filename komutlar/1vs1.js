...
exports.run = async (client, message, args) => {
  ...
  let modlog = guild.channels.find('name', 'oyun');
  let reason = args.slice(1).join(' ');
  let user = message.mentions.users.first();
  if (!modlog) return message.reply('`oyun` kanalı oluşturman lazım.Sohbeti kirletmeyelim :) ');
	let opponent = message.mentions.users.first()
	if (!opponent) return message.reply("TRCyber|Oynamak istediğin kişiyi etiketlemelisin!")
